...
// Get hourly report for specific client (shared across all users)
router.get('/client/:clientId', (req, res) => {
  ...
  // Verify client exists (shared across all users)
  db.get(
    'SELECT id, name, user_email FROM clients WHERE id = ?',
    [clientId],
    (err, client) => {
      if (err) {
      ...
      // Get work entries for this client (shared across all users)
      db.all(
        `SELECT id, hours, description, date, user_email, created_at, updated_at
         FROM work_entries 
         WHERE client_id = ? 
         ORDER BY date DESC`,
        [clientId],
...
// Export client report as CSV (shared across all users)
router.get('/export/csv/:clientId', (req, res) => {
  ...
  db.get(
    ...
    (err, client) => {
      ...
      // Get work entries (shared across all users)
      db.all(
        `SELECT hours, description, date, user_email, created_at
         FROM work_entries 
         WHERE client_id = ? 
         ORDER BY date DESC`,
        [clientId],
...
// Export client report as PDF (shared across all users)
