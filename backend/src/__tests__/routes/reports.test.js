...
describe('Report Routes', () => {
  ...
  describe('PDF Export Success Path', () => {
    ...
    test('should handle work entry without description', async () => {
      ...
      expect(response.status).toBe(200);
      expect(mockPdfInstance.text).toHaveBeenCalledWith('No description', 230, expect.any(Number), { width: 300 });
    });

    test('should calculate total hours correctly in PDF', async () => {
      const mockClient = { id: 1, name: 'Test Client' };
      const mockWorkEntries = [
        { date: '2024-01-01', hours: 5.5, description: 'Work 1', created_at: '2024-01-01' },
        { date: '2024-01-02', hours: 3.25, description: 'Work 2', created_at: '2024-01-02' }
      ];
      ...
      expect(response.status).toBe(200);
      expect(mockPdfInstance.text).toHaveBeenCalledWith('Total Hours: 8.75');
    ...
    test('should sanitize client name in PDF filename', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.headers['content-disposition']).toContain('Test_Client_Special_');
    });
  });
});
