...
describe('Report Routes', () => {
  ...
  describe('PDF Export Success Path', () => {
    ...
    test('should handle work entries with null description in PDF', async () => {
      const mockClient = { id: 1, name: 'Test Client' };
      const mockWorkEntries = [
    ...
    test('should handle many work entries requiring page break in PDF', async () => {
      const mockClient = { id: 1, name: 'Test Client' };
      const mockWorkEntries = [];
      for (let i = 0; i < 50; i++) {
    ...
    test('should sanitize client name with special characters in PDF filename', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.headers['content-disposition']).toContain('Test_Client_Name_Special_report_');
    });
  });
    ...
    test('should sanitize client name with special characters in CSV filename', async () => {
      ...
      expect(csvWriter.createObjectCsvWriter).toHaveBeenCalledWith(
        expect.objectContaining({
          path: expect.stringContaining('Test_Client_Name_Special_report_')
        })
      );
    });
