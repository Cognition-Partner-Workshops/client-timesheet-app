  ...
  return jest.fn().mockImplementation(() => {
    ...
    return {
      ...
      pipe: jest.fn((stream) => {
        ...
        // Simulate PDF content being written and stream ending
        if (pipedStream) {
          pipedStream.end(Buffer.from('%PDF-1.4 mock pdf content'));
        }
      }),
      y: 100
    };
...
describe('Report Routes', () => {
  ...
  describe('PDF Export Success Path', () => {
    ...
    test('should generate PDF successfully with work entries', async () => {
      ...
      // PDF should be generated and piped to response
    ...
    test('should generate PDF with many entries triggering page break', async () => {
      const mockClient = { id: 1, name: 'Busy Client' };
      // Create many entries to trigger page breaks (y > 700)
  ...
  describe('CSV Export Edge Cases', () => {
    test('should handle special characters in client name for CSV filename', async () => {
      ...
      // Verify the flow handles special characters
