...
describe('Database Initialization', () => {
  ...
  describe('initializeDatabase', () => {
    ...
    test('should create indexes for performance', async () => {
      ...
      expect(queries.some(q => q.includes('CREATE INDEX IF NOT EXISTS idx_clients_name'))).toBe(true);
  ...
  describe('Database Schema', () => {
    ...
    test('clients table should be global (no user_email foreign key)', async () => {
      const db = getDatabase();
      await initializeDatabase();
      ...
      expect(clientTableQuery[0]).toContain('name TEXT NOT NULL UNIQUE');
      expect(clientTableQuery[0]).toContain('created_by TEXT');
      expect(clientTableQuery[0]).not.toContain('user_email TEXT NOT NULL');
      expect(clientTableQuery[0]).not.toContain('FOREIGN KEY (user_email) REFERENCES users (email) ON DELETE CASCADE');
    });
