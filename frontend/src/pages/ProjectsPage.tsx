...
const ProjectsPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: '', description: '', clientId: 0 });
  ...
  const createMutation = useMutation({
    mutationFn: (projectData: { name: string; description?: string; clientId: number }) =>
                ...
                {projects.length > 0 ? (
                  projects.map((project: Project) => (
                    <TableRow key={project.id}>
                      ...
                      <TableCell>
                        <Chip label={project.client_name} color="primary" variant="outlined" size="small" />
                      </TableCell>
                      <TableCell>
                        {project.description ? (
