...
class ApiClient {
  ...
  async createClient(clientData: { name: string; description?: string; email?: string; mobile_no?: string }) {
    const response = await this.client.post('/api/clients', clientData);
    return response.data;
  }

  async updateClient(id: number, clientData: { name?: string; description?: string; email?: string; mobile_no?: string }) {
